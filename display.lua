			...
			for slot_num = 1, 150 do
				...
				if not slot_already_processed then
					...
					if item_in_bag ~= nil then
						if
							item_in_bag.name == gear_item_needed.name
							and item_in_bag.itemGrade == gear_item_needed.grade
						then
...
local delay = 0
function DISPLAY.CreateDisplays(settings, toggleSettingsFunc)
	DISPLAY.CreateMainDisplay(settings, toggleSettingsFunc)
end
function DISPLAY.CreateMainDisplay(settings, toggleSettingsFunc)
	buttons = {}
	gear_to_process = {}
	...
	if canvas_x ~= 100 and canvas_y ~= 0 then
		Canvas:AddAnchor(
			"TOPLEFT",
			"UIParent",
			canvas_x * api.Interface:GetUIScale(),
			canvas_y * api.Interface:GetUIScale()
		)
	else
		Canvas:AddAnchor(
			"LEFT",
	...
	-- Force-shrunk Settings Button
	local configBtn = Canvas:CreateChildWidget("button", "hotSwap.configBtn", 0, true)
	configBtn:Show(true)
	configBtn:SetExtent(20, 20)
	configBtn:AddAnchor("TOPRIGHT", Canvas, -35, 7)
	api.Interface:ApplyButtonSkin(configBtn, BUTTON_BASIC.RESET)
	configBtn:SetHandler("OnClick", function()
		if toggleSettingsFunc then
			toggleSettingsFunc()
		end
	end)
	...
	closeBtn:Show(true)
	closeBtn:SetExtent(20, 20)
	closeBtn:AddAnchor("TOPRIGHT", Canvas, -10, 7)
	...
	showBtn:Show(false)
	showBtn:SetExtent(20, 20)
	showBtn:AddAnchor("TOPRIGHT", Canvas, -10, 7)
